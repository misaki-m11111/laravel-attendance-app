...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/admin/attendance_request/show.css') }}">
@endsection

@section('content')
    @php
        $attendanceDate = \Carbon\Carbon::parse(
            $attendanceRequest->attendance->attendance_date
        );
    @endphp
    ...
    <div class="attendance-detail">
        ...
        <div class="attendance-detail__card">
            ...
            @php
                $breakCount = max(
                    2,
                    $attendanceRequest->attendanceRequestBreaks->count()
                );
            @endphp

            @for ($i = 0; $i < $breakCount; $i++)
                @php
                    $requestBreak = $attendanceRequest
                ...
                <div class="attendance-detail__row">
                    ...
                    <div class="attendance-detail__value">
                        <div class="attendance-detail__time">
                            <span class="attendance-detail__time-text">
                                {{ $requestBreak
                                    && $requestBreak->requested_break_start
                                    ? \Carbon\Carbon::parse(
                                        $requestBreak->requested_break_start
                                    )->format('H:i')
                                    : '' }}
                            ...
                            <span class="attendance-detail__time-text">
                                {{ $requestBreak
                                    && $requestBreak->requested_break_end
                                    ? \Carbon\Carbon::parse(
                                        $requestBreak->requested_break_end
                                    )->format('H:i')
                                    : '' }}
        ...
        <div class="attendance-detail__button-area">
            ...
            @else
                <button
                    class="attendance-detail__button attendance-detail__button--approved"
                    type="button"
                    disabled
                >
                    承認済み
